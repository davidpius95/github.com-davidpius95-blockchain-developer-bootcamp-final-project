...
const yesu = artifacts.require("./yesu.sol");
...
contract("Crypto Boys", async (accounts) => {
  let yesu, result, cryptoBoyCount;

  before(async () => {
    yesu = await yesu.deployed();
  });

  describe("Deployment", async () => {
    it("contract has an address", async () => {
      const address = await yesu.address;
    ...
    it("has a name", async () => {
      const name = await yesu.collectionName();
      assert.equal(name, "Crypto Boys Collection");
    });

    it("has a symbol", async () => {
      const symbol = await yesu.collectionNameSymbol();
  ...
  describe("application features", async () => {
    it("allows users to mint ERC721 token", async () => {
      cryptoBoyCount = await yesu.cryptoBoyCounter();
      assert.equal(cryptoBoyCount.toNumber(), 0);

      let tokenExists;
      tokenExists = await yesu.getTokenExists(1, { from: accounts[0] });
      assert.equal(tokenExists, false);

      let tokenURIExists;
      tokenURIExists = await yesu.tokenURIExists(
      ...
      let tokenNameExists;
      tokenNameExists = await yesu.tokenNameExists("myCBNFT", {
        from: accounts[0],
      });
      ...
      for (let i = 0; i < colorsArray1.length; i++) {
        colorExists = await yesu.colorExists(colorsArray1[i], {
          from: accounts[0],
        });
        assert.equal(colorExists, false);
      }

      result = await yesu.mintCryptoBoy(
        "myCBNFT",
        "https://gateway.pinata.cloud/ipfs/QmYFmJgQGH4uPHRYN15Xdv4aLd9o4Aq63y1e4GgN6kj5aK/2",
      ...
      let cryptoboy;
      cryptoboy = await yesu.allyesu(1, {
        from: accounts[0],
      });
    ...
    it("returns total number of tokens minted so far", async () => {
      const totalNumberOfTokensMinted = await yesu.getNumberOfTokensMinted();
      assert.equal(totalNumberOfTokensMinted.toNumber(), 4);
    });

    it("returns total number of tokens owned by an address", async () => {
      const totalNumberOfTokensOwnedByAnAddress = await yesu.getTotalNumberOfTokensOwnedByAnAddress(
        accounts[0]
      );
    ...
    it("allows users to buy token for specified ethers", async () => {
      const oldTokenOwner = await yesu.getTokenOwner(1);
      ...
      oldTotalNumberOfTokensOwnedBySeller = await yesu.getTotalNumberOfTokensOwnedByAnAddress(
        accounts[0]
      );
      ...
      result = await yesu.buyToken(1, {
        from: accounts[2],
        value: web3.utils.toWei("1", "Ether"),
      });

      const newTokenOwner = await yesu.getTokenOwner(1);
      ...
      newTotalNumberOfTokensOwnedBySeller = await yesu.getTotalNumberOfTokensOwnedByAnAddress(
        accounts[0]
      );
      ...
      await yesu.buyToken(2, {
        from: 0x0000000000000000000000000000000000000000,
        value: web3.utils.toWei("1", "Ether"),
      }).should.be.rejected;

      await yesu.buyToken(56, {
        from: accounts[4],
        value: web3.utils.toWei("1", "Ether"),
      }).should.be.rejected;

      await yesu.buyToken(3, {
        from: accounts[0],
        value: web3.utils.toWei("1", "Ether"),
      }).should.be.rejected;
    });
    ...
    it("allows users to change token price", async () => {
      let cryptoBoyPrice;
      cryptoBoyPrice = await yesu.allyesu(1, {
        from: accounts[0],
      });
      assert.equal(web3.utils.fromWei(cryptoBoyPrice.price, "ether"), 1);

      result = await yesu.changeTokenPrice(
        1,
      ...
      await yesu.changeTokenPrice(1, web3.utils.toWei("3", "Ether"), {
        from: 0x0000000000000000000000000000000000000000,
      }).should.be.rejected;

      await yesu.changeTokenPrice(82, web3.utils.toWei("3", "Ether"), {
        from: accounts[2],
      }).should.be.rejected;
    ...
    it("allows users to toggle between setting the token for sale or not for sale", async () => {
      ...
      result = await yesu.toggleForSale(1, { from: accounts[2] });
      ...
      await yesu.toggleForSale(1, {
        from: 0x0000000000000000000000000000000000000000,
      }).should.be.rejected;

      await yesu.toggleForSale(94, { from: accounts[2] }).should.be
        .rejected;

      await yesu.toggleForSale(1, { from: accounts[8] }).should.be
        .rejected;
    });
  });
});
